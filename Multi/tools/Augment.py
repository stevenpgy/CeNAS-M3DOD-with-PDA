...
def apply_random_operation(image, operation, bbox_left, bbox_top, bbox_right, bbox_bottom):
    ...
    # Calculate the coordinates for the nine equal parts
    ...
    for i in range(3):
        for j in range(3):
            left = bbox_left + j * third_width
            top = bbox_top + i * third_height
            right = min(bbox_left + (j + 1) * third_width, bbox_right)
            bottom = min(bbox_top + (i + 1) * third_height, bbox_bottom)
            if right > left and bottom > top:
                regions.append((left, top, right, bottom))

    # Ensure there is at least one valid region
    if not regions:
        return image
...
def draw_2d_bounding_boxes_with_random_operations(image, labels):
    for label in labels:
        ...
        # Apply the selected operation to a random ninth of the bounding box
...
output_directory = './datasets/coco/TrainImgAug_8*3_hfull'
